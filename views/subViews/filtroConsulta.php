...
<section class="formInit-bg aala ativo" id="b">
    <div class="form-bg">
        <h1>Alterar/Excluir Consultas</h1>
        <form action="../../views/administracao.php" id="FormFiltro" method="POST">
            <div id="styleForm">
                ...
                <div>
                    ...
                    <select name="filtro" id="filtro">
                        <option value="id_consulta_PK" selected>Id</option>
                        <option value="nomeFarmaceutico" >Nome Farmacêutico</option>
                        <option value="nomePaciente">Nome Paciente</option>
                        <option value="remediosPrescritos">Remedios Prescritos</option>
                        <option value="posologia">posologia</option>
                        <option value="dataConsulta">Data Consulta</option>
                        <option value="horaConsulta">Hora Consulta</option>
                    </select>
                </div>
                <div>
        ...
        <div class="styleForm">
            <div class="tabela-container">
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    $resultados = $FiltroObj->buscarValoresConsultasSemelhantes($input, $filtro, $_SESSION["user_id"]);

                    if ($resultados !== false && !empty($resultados)) {
                        echo '<table >
                                <tr>
                                <th>Id</th>
                                <th>Nome Farmacêutico</th>
                                <th>Nome Paciente</th>
                                <th>Remedios Prescritos</th>
                                <th>Posologia</th>
                                <th>Data Consulta</th>
                                <th>Hora Consulta</th>
                                <th>Selecionar</th>
                        ...
                        foreach ($resultados as $pessoa) {
                            echo '<tr>';
                            echo '<td>' . $pessoa['id_consulta_PK'] . '</td>';
                            echo '<td>' . $pessoa['nomefarmaceutico'] . '</td>';
                            echo '<td>' . $pessoa['nomePaciente'] . '</td>';
                            echo '<td>' . $pessoa['remediosPrescritos'] . '</td>';
                            echo '<td>' . $pessoa['Posologia'] . '</td>';
                            echo '<td>' . $pessoa['dataConsulta'] . '</td>';
                            echo '<td>' . $pessoa['horaConsulta'] . '</td>';
                            echo '<td><input type="checkbox" class="checkboxFiltro" id=' . $pessoa['id_consulta_PK'] . '></td>';
                            echo '</tr>';
                        }
...
<div id="meuModalExcluir" class="modal" style="z-index: 99999;">
    ...
    <div class="modal-conteudo" style="width: 40%;">
        ...
        <div style="width: 100%; display: flex;justify-content: center;align-items: center;">
            <form id="formMensagem" action="../controllers/controllerExcluirConsulta.php" method="post" style="width: 100%;">
